...
public class MeleeEnemyAI : MonoBehaviour, IDamageable
{
    ...
    IEnumerator Melee()
    {
        ...
        GameObject meleeClone = Instantiate(hit, hitSpawnPos.transform.position, hit.transform.rotation);
        meleeClone.GetComponent<MeleeHit>().damage = damage;
        meleeClone.GetComponent<MeleeHit>().speed = speed;
        meleeClone.GetComponent<MeleeHit>().destroyTime = meleeDestroyTime;
